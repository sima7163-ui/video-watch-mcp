  ...
  async fetch(request, env) {
    ...
    if (request.method === 'POST' && url.pathname === '/sync') {
      ...
      try {
        ...
        const heartRate = body.heart_rate ? parseFloat(body.heart_rate) : null;

        await env.DB.prepare(`
          INSERT INTO health_logs (date, sleep_duration_minutes, sleep_start, sleep_end, sleep_quality,
            cycle_day, last_period_start, cycle_length_avg, hrv, resting_hr, steps, active_energy, wrist_temperature, heart_rate, notes)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).bind(
          today, sleepMinutes,
          body.sleep_start || null, body.sleep_end || null,
          quality, cycleDay, lastPeriodStart, cycleLength,
      ...
      try {
        ...
        if (body.method === 'tools/list') {
          return cors(new Response(JSON.stringify({
            ...
            result: {
              tools: [{
                name: 'health_check',
                description: 'Vráti aktuálny zdravotný stav Simi — spánok, fáza cyklu, riziko bolesti hlavy, HRV, srdcová frekvencia. Volaj vždy keď Simi spomína únavu, bolesť hlavy, náladu alebo keď ju chceš lepšie chápať.',
                inputSchema: {
                  type: 'object',
                  properties: {
        ...
        if (body.method === 'tools/call' && body.params?.name === 'health_check') {
          ...
          const lines = rows.results.map(r => {
            ...
            if (r.heart_rate) parts.push(`Srdcová frekvencia: ${r.heart_rate} BPM`);
